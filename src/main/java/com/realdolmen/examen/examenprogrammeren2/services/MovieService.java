...
public class MovieService {
     ...
     *
     * @throws NoQueryPossibleException when the repository can't handle the
     * query
     ...
     * @throws NoQueryPossibleException when the repository can't handle the
     * query
     * @return {@link List} resulting set of {@link Movie} or null if the
     * {@link Movie} was not found
     */
    ...
    /**
     * @return {@link List} resulting set of {@link String} or empty if no
     * {@link Movie} was not found with a palindrome as title
     */
    ...
    protected boolean isAPalindrome(String s) {
     ...
     /**
     * @param movie {@Link movie) to be added to the DB
     * @return {@link Movie) with an id received from the database
     */
    public Movie insertNewMovie(Movie movie) throws NoQueryPossibleException {
        try {
            boolean isInsert = movieRepository.create(QueryHelper.createNewMovie(movie));
            int max = movieRepository.max();
            return findMovieById(max);
